...
<script type="text/javascript">
	...
	var wpImport = {
		start_import_not_home: function() {
			if(confirm("<?php echo t("Are you sure that's where you want your blog posts and pages to go?"); ?>"))
...
<?php } ?>
		...
		<div id="form_box">
			...
			<h2><?php echo t("Select a page for your WordPress blog posts to be imported under.") ?></h2>
			<div style="width:400px">
			<?php 
			if (intval(Page::getByPath('/blog')->getCollectionID())>0){
				$cID = Page::getByPath('/blog')->getCollectionID();
			} else {
				$cID = 1;
			}
			echo FormPageSelectorHelper::selectPage('new-root-posts', $cID); ?>
			</div>
			<h2><?php echo t('Choose a Page Type for WordPress "Posts":') ?></h2>
			<?php echo $form->select('wordpress-blogs', $collectiontypes); ?>
			<br />&nbsp;<br/>
			<h2><?php echo t("Select a page for your WordPress pages to be imported under.") ?></h2>
			<div style="width:400px">
			<?php echo FormPageSelectorHelper::selectPage('new-root-pages', 1); ?>
			</div>
